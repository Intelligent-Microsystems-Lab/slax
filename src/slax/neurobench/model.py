...
class NeuroBenchModel:
    ...
    def activation_layers(self):
        ...
        def get_activation_layers(parent):
            ...
            layers = []
            children = list(parent.iter_modules())[:-1]
            for child in children:
                if check_if_activation(child[1]):
                    # is an activation module
                    layers.append(child[1])
                else:
                    if len(list(child[1].iter_modules())) != 1:
                        print(len(list(child[1].iter_modules())))
                        # not an activation module and has nested submodules
                        children_layers = get_activation_layers(child[1])
    ...
    def connection_layers(self):
        ...
        def get_connection_layers(parent):
            ...
            children = list(parent.iter_modules())[:-1]
            for child in children:
                grand_children = list(child[1].iter_modules())[:-1]
                if len(grand_children) == 0:  # leaf child
                    if isinstance(child[1], supported_layers):
                        connection_layers.append(child[1])

                else:
                    children_layers = get_connection_layers(child[1])
