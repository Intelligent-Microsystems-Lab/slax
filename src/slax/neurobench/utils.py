...
def make_binary_copy(layer, all_ones=False):
    ...
    recurrent_cells = Neuron#torch.nn.RNNCellBase

    if isinstance(layer, stateless_layers):
        #weights = layer_copy.weight.data
        weights = layer_copy.kernel.value
        weights[weights != 0] = int(1)
        if all_ones:
            weights[weights == 0] = int(1)

        if layer.use_bias:
            biases = layer_copy.bias.value
            biases[biases != 0] = int(1)
            if all_ones:
                biases[biases == 0] = int(1)
            layer_copy.bias.value = biases

        layer_copy.kernel.value = weights
