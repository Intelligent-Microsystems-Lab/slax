...
def activation_sparsity(model, preds, data):
    ...
    filter = lambda mdl, method_name: isinstance(mdl, sl.Neuron)
    return sparsity
...
def MSE(model, preds, data):
    ...
    return jnp.mean((preds - data[1])**2)
...
def sMAPE(model, preds, data):
    ...
    #check_shape(preds, data[1])
    smape = 200*jnp.mean(torch.abs(preds - data[1])/(jnp.abs(preds)+jnp.abs(data[1])))
    return jnp.nan_to_num(smape, nan=200.0)
...
class r2(AccumulatedMetric):
    ...
    def __init__(self):
        ...
        self.x_labels = jnp.array([])#torch.tensor([])
        self.y_labels = jnp.array([])#torch.tensor([])
    ...
    def __call__(self, model, preds, data):
        ...
        #check_shape(preds, data[1])
        self.x_sum_squares += jnp.sum((data[1][:, 0] - preds[:, 0])**2)
        self.y_sum_squares += jnp.sum((data[1][:, 1] - preds[:, 1])**2)
        self.x_labels = jnp.concatenate((self.x_labels, data[1][:, 0]))
        self.y_labels = jnp.concatenate((self.y_labels, data[1][:, 1]))
    ...
    def compute(self):
        ...
        x_denom = self.x_labels.var(ddof=0)*len(self.x_labels)
        y_denom = self.y_labels.var(ddof=0)*len(self.y_labels)
